...
(() => {
  ...
  const DB_VERSION = 2; // v2 adds the "parties" store for name autosuggest/autofill
  let dbPromise = null;
  ...
  function openDB() {
    ...
    dbPromise = new Promise((resolve, reject) => {
      ...
      req.onupgradeneeded = (e) => {
        ...
        // "parties" stores one profile per unique party name so the form
        // can auto-suggest and auto-fill address / mobile / GSTIN / agent
        // the next time the same party name is typed in.
        if (!db.objectStoreNames.contains("parties")) {
          db.createObjectStore("parties", { keyPath: "key" }); // key = normalized (lowercased, trimmed) party name
        }
      };
  ...
  const DB = {
    ...
    async getAllParties() {
      const store = await tx("parties", "readonly");
      return reqToPromise(store.getAll());
    },
    async putParty(party) {
      const store = await tx("parties", "readwrite");
      return reqToPromise(store.put(party));
    },
    async clearParties() {
      const store = await tx("parties", "readwrite");
      return reqToPromise(store.clear());
  ...
  // In-memory cache of saved party profiles, loaded once at startup and kept
  // in sync on save, so the name field can filter suggestions instantly on
  // every keystroke without hitting IndexedDB each time.
  let partyCache = [];
  ...
  function renderItems() {
    ...
    state.items.forEach((item, idx) => {
      ...
      row.innerHTML = `
        ...
        <span class="field-wrap"><label>Size Chart</label><input type="text" class="f-size" inputmode="tel" placeholder="e.g. 30x2,32x3,34x1" value="${escapeAttr(item.sizeChart)}" /></span>
  ...
  /* ---------------- Party autosuggest / autofill ----------------
     Saves a lightweight "party profile" (address / mobile / GSTIN / agent)
     under the party's name every time an order is saved. While typing the
     party name on later orders, matching profiles are suggested in a
     dropdown; picking one (or tabbing away from an exact match) fills in
     the rest of the party fields automatically. Only order-specific fields
     (delivery date, notes, items) are left untouched. */
  ...
  async function loadPartyCache() {
    try {
      partyCache = await DB.getAllParties();
    } catch (e) {
      console.warn("Could not load party list", e);
      partyCache = [];
    }
  }

  async function upsertPartyFromOrder(data) {
    const key = normalizePartyKey(data.partyName);
    if (!key) return;
    try {
      await DB.putParty({
        key,
      ...
      // Keep the in-memory cache in sync without a full reload.
      const idx = partyCache.findIndex((p) => p.key === key);
      const record = { key, name: data.partyName, partyAddress: data.partyAddress || "", partyMobile: data.partyMobile || "", partyGSTIN: data.partyGSTIN || "", agentName: data.agentName || "", updatedAt: Date.now() };
      if (idx >= 0) partyCache[idx] = record; else partyCache.push(record);
    } catch (e) {
      console.warn("Could not save party profile", e);
    }
  }

  function findPartyByExactName(name) {
    const key = normalizePartyKey(name);
    if (!key) return null;
    return partyCache.find((p) => p.key === key) || null;
  }

  function fillPartyDetails(party, { overwrite = true } = {}) {
    if (!party) return;
    const apply = (field, value) => {
      if (!value) return;
      if (overwrite || !field.value.trim()) field.value = value;
    };
    apply(el.partyAddress, party.partyAddress);
    apply(el.partyMobile, party.partyMobile);
    apply(el.partyGSTIN, party.partyGSTIN);
    apply(el.agentName, party.agentName);
  }
  ...
  function hideSuggestions() {
    el.partySuggestions.hidden = true;
    el.partySuggestions.innerHTML = "";
    suggestionItems = [];
  ...
  function renderSuggestions(matches) {
    ...
    el.partySuggestions.innerHTML = matches
      .map((p, i) => {
        const metaBits = [p.partyMobile, p.partyAddress].filter(Boolean);
        return `<li data-idx="${i}"><span class="ps-name">${escapeAttr(p.name)}</span>${metaBits.length ? `<span class="ps-meta">${escapeAttr(metaBits.join(" · "))}</span>` : ""}</li>`;
      })
      .join("");
    el.partySuggestions.hidden = false;
  }

  function showSuggestionsFor(query) {
    const q = normalizePartyKey(query);
    if (!q) return hideSuggestions();
    const matches = partyCache
      .filter((p) => p.key.includes(q))
      .sort((a, b) => {
        // Prefix matches first, then most recently used.
      ...
      .slice(0, 6);
    // Don't show a single suggestion that's already an exact match of what's typed.
    if (matches.length === 1 && matches[0].key === q) return hideSuggestions();
    renderSuggestions(matches);
  }

  function selectSuggestion(idx) {
    const party = suggestionItems[idx];
    if (!party) return;
    el.partyName.value = party.name;
    fillPartyDetails(party, { overwrite: true });
    hideSuggestions();
  }

  function setActiveSuggestion(idx) {
    const items = el.partySuggestions.querySelectorAll("li");
  ...
  el.partyName.addEventListener("input", () => {
    showSuggestionsFor(el.partyName.value);
  });

  el.partyName.addEventListener("focus", () => {
    if (el.partyName.value.trim()) showSuggestionsFor(el.partyName.value);
  });

  el.partyName.addEventListener("keydown", (e) => {
    if (el.partySuggestions.hidden) return;
    if (e.key === "ArrowDown") {
      e.preventDefault();
  ...
  el.partySuggestions.addEventListener("mousedown", (e) => {
  ...
  el.partyName.addEventListener("blur", () => {
    // Give a picked <li>'s mousedown a chance to run first, then close.
    setTimeout(() => {
      hideSuggestions();
      // Auto-fill on an exact name match even if the person never opened
      // the dropdown (e.g. they typed the full name from memory) — but
      // only into fields that are still empty, so it never clobbers
      // details they've already typed for this order.
      const exact = findPartyByExactName(el.partyName.value);
      if (exact) fillPartyDetails(exact, { overwrite: false });
    }, 150);
  });
  ...
  async function saveOrder() {
    ...
    await upsertPartyFromOrder(data);
  ...
  el.btnBackup.addEventListener("click", async () => {
    ...
    try {
      const [orders, meta, parties] = await Promise.all([DB.getAllOrders(), DB.getAllMeta(), DB.getAllParties()]);
      const payload = {
  ...
  el.restoreFileInput.addEventListener("change", async () => {
    ...
    try {
      ...
      // Older backups (backupVersion 1) predate the parties store — skip if absent.
      if (Array.isArray(payload.parties)) {
        for (const p of payload.parties) {
          if (p && p.key) await DB.putParty(p);
        }
      }
      await refreshBadge();
  ...
  // IMPORTANT: this runs immediately at script load, NOT inside init(),
  // because init() only runs once the password lock is passed — and by
  // that time (a human typing a password) the window "load" event has
  // almost always already fired once. addEventListener("load", ...) never
  // fires for an event that already happened, so registration was
  // effectively skipped on every normal (locked) visit. Registering here,
  // with a fallback for the case where "load" already fired, ensures the
  // app shell actually gets cached so offline mode works.
